...
int get_data()// function to get input
{
int n,i,flag;
...
int get_index(int time,int n)// function to get index according to highest response time within time="time"
{   
  int index=-1,flag=0,i;
  float max=-1,hrn,time_=time,arr,s;
  ...
  while(1)
  {
   ...
   for(i=1;i<=n;i++)
    {
         if(arrival_time[i]<=time && hash_array[i]==0) // check if process is within given time and process has not been performed yet
          {                                            // using hash_array
            arr=arrival_time[i];
            s=service_time[i];
            hrn=((time_-arr)+s)/s;
...
void display(int n,float avg)
{
  int i;
  ofstream obj;
  obj.open("/home/dell1/OS_LAb/Lab3/hrn_graph.txt");
